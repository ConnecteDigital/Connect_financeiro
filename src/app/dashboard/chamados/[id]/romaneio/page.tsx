...
function RomaneioContent({ id }: { id: string }) {
  ...
  return (
    <>
      {/* Back link */}
      <div className="print:hidden flex items-center max-w-[440px] mx-auto pt-3 pb-2 px-3">
      ...
      <div
        ref={sheetRef}
        className="romaneio-sheet bg-white mx-auto p-6 pb-6"
        style={{ maxWidth: 440, fontFamily: 'Arial, sans-serif' }}
      >
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {logoUrl ? (
              ...
              <div style={{ width: 56, height: 56, background: '#f3f4f6', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontWeight: 900, fontSize: 20, color: '#374151' }}>{companyName.charAt(0)}</span>
              </div>
            )}
            <p style={{ fontWeight: 700, fontSize: 13, lineHeight: 1.3 }}>{companyName}</p>
          </div>
          <div style={{ textAlign: 'right', fontSize: 11, lineHeight: 1.8 }}>
            <p><strong>CHAMADO:</strong> {number}</p>
...
function LineField({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div style={{ borderBottom: '1.5px solid black', paddingBottom: 3, minHeight: 22 }}>
